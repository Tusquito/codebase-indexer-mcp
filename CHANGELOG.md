...
- **Model-accurate dense truncation** ([ADR 0017](docs/adr/0017-model-tokenizer-tei-dense-truncation.md)) — dense Ollama truncation now uses the HuggingFace model tokenizer (`DENSE_EMBED_MODEL`) instead of BM25 word-split approximation; set `HF_HOME` for persistent tokenizer cache in air-gapped deployments
