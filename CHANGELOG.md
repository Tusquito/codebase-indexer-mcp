...
- **Host and Proxy tests on TUnit** ([ADR 0034](docs/adr/0034-migrate-unit-tests-to-tunit.md) Phase 2) — Host and Proxy test projects use TUnit (including AspNetCore `WebApplicationTest` host fixtures) instead of xUnit/VSTest; AppHost tests remain deferred to Phase 3
- **.NET unit tests on TUnit** ([ADR 0034](docs/adr/0034-migrate-unit-tests-to-tunit.md) Phase 1) — Domain / Application / Infrastructure tests use TUnit + TUnit.Mocks on Microsoft.Testing.Platform; xUnit / VSTest SDK / Coverlet removed from shared test props; Host / Proxy restored in Phase 2; AppHost still out of solution until Phase 3
- **MCP unified error envelope** ([ADR 0033](docs/adr/0033-adopt-result-pattern.md) Phase 3) — expected failures from storage/embed ports and MCP tools return `{ "error": { "kind", "code", "message", "metadata?" } }` with PascalCase `kind` (e.g. `Validation`); one-off error DTOs removed (`IndexAlreadyRunningResponse`, path/job-not-found, chunk/outline/summary errors); empty search hit lists remain success; soft-read Qdrant transport failures surface as Dependency (not empty-success)
