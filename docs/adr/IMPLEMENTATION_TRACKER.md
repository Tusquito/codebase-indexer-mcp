...
| [0025](0025-huggingface-tei-dense-embedding.md) | Adopt HuggingFace TEI sidecar for dense embedding | Accepted | Phase 1 — TEI hard replace (final phase) — closeout | `verified` | `TeiDenseBackend` + OpenAI `/v1/embeddings`; TEI compose (`docker-compose.tei.yml` + `.tei.gpu.yml`, profile `bundled-tei`); Ollama dense deleted; closeout complete: Ollama→TEI doc/docstring sweep (16 files); model ids normalized to canonical HF repo ids (`jinaai/jina-embeddings-v2-base-code`); GPU verification via `docker exec codeindexer_tei nvidia-smi`; upstream TEI CUDA-detection bug fixed via `docker-compose.tei.gpu.yml` entrypoint override; live GPU quality-validation (recall@10=0.3590, MRR=0.3576, ndcg@10=0.2807, threshold 0 self-compare pass); `eval_baseline.json` + ADR 0025 Measured outcomes refreshed; 393 unit tests pass (8 skipped); review rounds: 2; `benchmarks/train/**` excluded (ADR 0020 follow-up); awaiting merge | 2026-07-07 |
