...
All config is env-var driven via `.env` (copy from `.env.example`) plus Aspire `Section__Property` overrides. **Production runtime is .NET** (`src/CodebaseIndexer.Host`, `docker-compose.aspire.yml`). Config sections live in `appsettings.json` (`Qdrant`, `Tei`, `Embedding`, `Workspace`, `Chunking`, `Indexing`, `Colbert`, `Graph`, `Reindex`, `Discovery`); override with `Section__Property` (e.g. `Qdrant__Url`, `Embedding__DenseModel`). Compose-only caps: `WORKSPACE_ROOT`, `MCP_MEM_LIMIT`, `QDRANT_MEM_LIMIT`, `MCP_CPUS`, `QDRANT_CPUS`, `ACCELERATOR`, `TEI_IMAGE`, `ASPIRE_FASTEMBED_CACHE`. Flat Python-style names (`DENSE_EMBED_MODEL`, `TEI_URL`, …) remain for compose/harness convenience and map to `Embedding__*` / `Tei__*`. Aspire pairs `TEI_MAX_BATCH_TOKENS` / `Embedding__MaxDenseTokens` @ 1024 ([ADR 0035](docs/adr/0035-tei-max-batch-tokens-client-pairing.md)). See [ADR 0030](docs/adr/0030-migrate-mcp-server-to-dotnet10.md), [ADR 0025](docs/adr/0025-huggingface-tei-dense-embedding.md).
