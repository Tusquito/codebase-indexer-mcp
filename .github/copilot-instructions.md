...
All config is env-var driven via `.env` (copy from `.env.example`). Key vars: `WORKSPACE_ROOT`, `DENSE_EMBED_MODEL`, `SPARSE_EMBED_MODEL`, `HYBRID_SEARCH`, `MCP_TRANSPORT`. Performance/RAM knobs (all env-overridable, defaults tuned for 16 CPU / 16 GB): container caps `MCP_MEM_LIMIT`/`QDRANT_MEM_LIMIT`/`MCP_CPUS`/`QDRANT_CPUS`, threading `OMP_NUM_THREADS`/`DENSE_THREADS`/`SPARSE_THREADS`, batching `BATCH_SIZE`/`FLUSH_EVERY`/`UPSERT_BATCH`/`READAHEAD_BUFFER`/`MAX_EMBED_CHARS`, glibc `MALLOC_ARENA_MAX`/`MALLOC_TRIM_THRESHOLD_`, Qdrant storage `VECTORS_ON_DISK`/`SPARSE_ON_DISK`/`QUANTIZATION`/`MEMMAP_THRESHOLD_KB`, memory pressure `MEMORY_PRESSURE_WARN_PCT`/`MEMORY_PRESSURE_HALT_PCT`, and model lifecycle `RELEASE_MODELS_AFTER_INDEX`/`MODEL_IDLE_TIMEOUT`. See `.env.example` for presets.
