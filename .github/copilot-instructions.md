...
A fully self-hosted MCP server that indexes codebases into a local Qdrant vector database using **Ollama for dense embeddings** and in-process BM25 sparse search, then exposes semantic search tools to AI agents. Ollama must be reachable for indexing and search.
...
3. **Embedder** (`embedder.py`) — **Ollama HTTP dense** (`OLLAMA_EMBED_MODEL`, `OLLAMA_URL`) + sparse lexical BM25 (`SPARSE_EMBED_MODEL`, default `Qdrant/bm25`) in-process. Sparse encoding runs on CPU via fastembed ONNX. `MAX_DENSE_EMBED_TOKENS` caps text sent to Ollama (word-split approximation; `0` auto-detects from `DENSE_EMBED_MODEL` registry). `MAX_SPARSE_EMBED_TOKENS` (default `0` = no sparse truncation for BM25). `SEQUENTIAL_EMBED` forces sparse-then-dense during indexing. Sparse BM25 is a **class-level singleton** released via `Embedder.release_models()`; Ollama has no in-process dense model. **Memory guard**: cgroup pressure can force sequential sparse+dense embedding. **Idle-timeout**: releases sparse BM25 after `MODEL_IDLE_TIMEOUT` seconds of no embed activity.
